...
def lambda_handler(event, context):
    ...
    print(final_message)
    # =========================
    # === PHẦN MỚI: KHỞI ĐỘNG STEP FUNCTION ===
    if extracted_contents and STATE_MACHINE_ARN:
        try:
            # Input cho Step Function chính là list nội dung
            sfn_input = {
                "extracted_contents": extracted_contents
            }
            
            # Tạo tên duy nhất cho mỗi lần chạy
            execution_name = f"proofread-job-{uuid.uuid4()}"
            ...
            print(f"Đã khởi động thành công Step Function: {response['executionArn']}")
        
        except Exception as e:
            print(f"LỖI khi khởi động Step Function: {e}")
